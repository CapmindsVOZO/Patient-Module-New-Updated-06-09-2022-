...
//
//	
//		sendkeys(pom.getPatient().gettypefirstname(), "Christofer");
//
//		sendkeys(pom.getPatient().gettypelastname(), "k");
//
//		sendkeys(pom.getPatient().sendtypeemail(), "xyzrrr@gmail.com");
//
//		sendkeys(pom.getPatient().gettypephonenumber(), "9090908904");
//		
//		sendkeys(pom.getPatient().geClicktId(), "909094");
//		
//		Clickonelement(pom.getPatient().getClickProvider());
//		
//		Clickonelement(pom.getPatient().getSelectProvider());
//
...
//	Clickonelement(pom.getPatient().getflageditsave());
		...
		Clickonelement(pom.getPatient().getclickChrisclient());

		Thread.sleep(6000);

		// Clickonelement(pom.getPatient().getclickVitals1());

//		WebDriverWait wait4 = new WebDriverWait(driver, 10);
//		WebElement element4 = wait4
//				.until(ExpectedConditions.elementToBeClickable(By.xpath("//*[name()='path' and contains(@d,'M19 3H5c-1')]")));
//		((JavascriptExecutor) driver).executeScript("arguments[0].click();", element4);
		...
		Clickonelement(pom.getPatient().getClickCancel6());
...
//		// Clickonelement(pom.getPatient().getclickSaveButton());
		...
		// clear mobile
		ClearText(pom.getPatient().getclearMobile());
		...
		// clear email
		ClearText(pom.getPatient().getclearemail());
		...
		// clear state
		ClearText(pom.getPatient().getclearState());

		// Enter State
		sendkeys(pom.getPatient().getEnterState(), "TAMILNADU");

		// clear ZIP Code
		ClearText(pom.getPatient().getclearZIPCode());
...
//
//		Clickonelement(pom.getPatient().getClickflag());
//
//		Clickonelement(pom.getPatient().getselectflag());
//
//		// Enter Description
//		sendkeys(pom.getPatient().getsendmsg(), "HI ALL");
//
//		Clickonelement(pom.getPatient().getclciksave());
//
//		Thread.sleep(2000);
//		Clickonelement(pom.getPatient().getclcikEdit());
//
//		Clickonelement(pom.getPatient().getclcikcancel());
//////
//////		Thread.sleep(2000);
//
//		///////////////// click appointment////////////
...
///////////////////   click Bill and Insurance
		...
		// clcik bill and insu
		Clickonelement(pom.getPatient().getclickBillandInsu());

		// clcik bill and insu
		Clickonelement(pom.getPatient().getclickaddcard());

		// Enter card holder name
		sendkeys(pom.getPatient().getEnterCardholdername(), "Ragu");

		// Enter card num
		sendkeys(pom.getPatient().getEnterCardnum(), "4444 5555 6666 8888");

		// Enter card CVV
		sendkeys(pom.getPatient().getEnterCvv(), "888");

		// Enter card expiry date
		sendkeys(pom.getPatient().getEnterMMYY(), "1212");
...
//		Clickonelement(pom.getPatient().getedipres());
	...
	WebElement fla5 = wait7
			.until(ExpectedConditions.elementToBeClickable(By.xpath("//a[normalize-space()='Client Portal Access']")));
	((JavascriptExecutor) driver).executeScript("arguments[0].click();", fla5);

	Thread.sleep(4000);
	
	// Click Provider Drop down
	Clickonelement(pom.getPatient().getClickEnablePatientPortal());
	
	// Click Revoke
	Clickonelement(pom.getPatient().getClickRevoke());
...
//search patient
		...
		sendkeys(pom.getPatient().getsendmsg(), "Hello ");
...
/////////// Form and Library
       ...
       // click form and library
		Clickonelement(pom.getPatient().getcliFormandLib());

       // Enter specialities
		sendkeys(pom.getPatient().getEnterspecia(), "Registry ");

      //Enter Form
		sendkeys(pom.getPatient().getEnterspecia(), "Registration Form ");

//////////// Macro Button

		// click form and library
		Clickonelement(pom.getPatient().getcliMacro());

		// Enter Label
		sendkeys(pom.getPatient().getEnterlabel(), "hi ");

		// Enter Generated Text
		sendkeys(pom.getPatient().getEnterGeneratedText(), "good Form ");
...
////Lab order	
		...
		// Click search button
		// Clickonelement(pom.getPatient().getClickLaborder());

		// Click date
		Clickonelement(pom.getPatient().getClickDate());
		
		Thread.sleep(2000);
		
		// select date
		Clickonelement(pom.getPatient().getSelectDate());
		
		Thread.sleep(2000);
		
		// Click to date
		Clickonelement(pom.getPatient().getClickToDate());
		
		// select date
		//Clickonelement(pom.getPatient().getSelectToDate());
		
		// Click Drop down
		Clickonelement(pom.getPatient().getClickDropdown());
		
		// select in  Drop down
		Clickonelement(pom.getPatient().getSelectDropdown());
				
		// Click Provider Drop down
		Clickonelement(pom.getPatient().getClickProDropdown());
				
		// select Provider in  Drop down
		//Clickonelement(pom.getPatient().getSelectProDropdown());
